...
let handler = async (m, { conn, text }) => {
    if (text.match(/(https:\/\/sfile.mobi\/)/gi)) {
        ...
        if (!res) throw '❌ لا يمكن تحميل الملف'
        ...
        if (!res.length) throw `❌ لم يتم العثور على نتائج لـ "${text}"`
        res = res.map((v) => `*العنوان:* ${v.title}\n*الحجم:* ${v.size}\n*الرابط:* ${v.link}`).join`\n\n`
        m.reply(res)
    } else return m.reply('❌ أدخل كلمة بحث أو رابط Sfile')
}
handler.help = ['sfile']
...
async function download(url) {
    ...
    return { filename, mimetype, upload_at: up_at, uploader, total_download: total_down, download }
}
