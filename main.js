...
const connectionOptions = {
  ...
  connectTimeoutMs: 60000, defaultQueryTimeoutMs: 0, generateHighQualityLinkPreview: true, syncFullHistory: false, markOnlineOnConnect: true, keepAliveIntervalMs: 30000
}
...
async function connectionUpdate(update) {
  ...
  if (lastDisconnect && lastDisconnect.error && lastDisconnect.error.output && lastDisconnect.error.output.statusCode !== DisconnectReason.loggedOut && conn.ws.readyState !== CONNECTING) {
    ...
    console.log(chalk.yellow(`🔁 كود الانقطاع: ${statusCode} - إعادة الاتصال بعد 5 ثوانٍ...`))
  ...
  conn.ev.on('contacts.update', (updates) => {
    ...
    for (const update of updates) {
      if (update.lid && update.id && !update.id.endsWith('@lid')) {
        conn.isLid[update.lid] = update.id
      }
    }
  })
  isInit = false
